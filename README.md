...
Windows packaging includes `cachelayer_tools/run_mcp.cmd` and `run_verify_hook.cmd`. Cursor's hook/MCP manifest format does not provide a portable per-OS command selector, so the managed remote MCP remains usable on Windows while local hooks require Git Bash or an explicit user-level command override to the bundled `.cmd` launcher. Failures remain fail-open.
